...
<h1>Data <strong>Tarik Saldo</strong></h1>
        ...
        @endphp
          @foreach ($data as $item)
            <tr>
                ...
                <td>{{ $item->nasabah->no_rekening }}</td>
                <td>{{ $item->nasabah->nama }}</td>
                <td>{{ $item->nasabah->nis }}</td>
                <td>{{ $item->nasabah->status }}</td>
                <td>Rp {{ number_format($item->jumlah) }}</td>
                @can('isAdmin')
                <td>
                    <a href="{{ route('tarik.delete',$item->id) }}" class="btn btn-danger" onclick="return confirm('Yakin Anda Ingin Menghapus?')"><i class="bi bi-trash"></i></a>
                </td>
                @elsecan('isTeller')
                @endcan
            </tr>
...
<div class="modal fade" id="nasabahModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{ route('tarik.create') }}" method="post">
            @csrf
            <div class="modal-body">
                <div class="input-group mt-2">
                    <span class="input-group-text">No Rekening</span>
                    <input type="hidden" name="transaksi" value="{{ $hasil }}">
                    <select class="form-select" aria-label="Default select example" name="id_nasabah">
                        <option selected>.:: Pilih Rekening ::.</option>
                        @foreach ($nasabah as $rekening)
                            <option value="{{ $rekening->id }}" aria-required="true">{{$rekening->no_rekening}} | {{$rekening->nama}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="input-group mt-2">
